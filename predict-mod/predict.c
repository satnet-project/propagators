		...
		if ((strcmp(sat[z].name,satname)==0) || (atol(satname)==sat[z].catnum))
		{
			...
			if ((start>=now-31557600) && (start<=now+31557600) && end==0)
			{
				...
				if (Decayed(indx,daynum)==0) {
					fprintf(fd,"%.0f\t %0.6f\t %0.6f\t %c\n", floor(86400.0*(3651.0+daynum)), sat_ele, sat_azi, findsun); 
/*					fprintf(fd,"%ld %s %.6f %.6f %4d %4d %4d %6ld %6ld %c\n",start,Daynum2String(daynum),sat_ele,sat_azi,ma256,isplat,isplong,irk,rv,findsun);*/
				}
				break;
			}
		...
		if ((strcmp(sat[z].name,satname)==0) || (atol(satname)==sat[z].catnum))
		{
			...
			if ((start>=now-31557600) && (start<=now+31557600))
			{
				...
				if (AosHappens(indx) && Geostationary(indx)==0 && Decayed(indx,daynum)==0)
				{
					...
					if (iel>=0)
					{
						printf("Predict - Satellite over the horizon.\n");
					}
					while (iel>=0)
					{
/*						fprintf(fd,"%.0f %s %0.6f %0.6f %4d %4d %4d %6ld %6ld %c\n",floor(86400.0*(3651.0+daynum)),Daynum2String(daynum),sat_ele,sat_azi,ma256,isplat,isplong,irk,rv,findsun);
*/
						fprintf(fd,"%.0f\t %0.6f\t %0.6f\t %c\n", floor(86400.0*(3651.0+daynum)), sat_ele, sat_azi, findsun); 					
						lastel=iel;
						daynum+=0.00001157407;
/*						daynum+=cos((sat_ele-1.0)*deg2rad)*sqrt(sat_alt)/25000.0;*/
						Calc();
					}
					...
					if (lastel!=0)
					{
						printf("Predict - Satellite below the horizon.\n");
						daynum=FindLOS();
						Calc();
/*						fprintf(fd,"%.0f %s %0.6f %0.6f %4d %4d %4d %6ld %6ld %c\n",floor(86400.0*(3651.0+daynum)),Daynum2String(daynum),sat_ele,sat_azi,ma256,isplat,isplong,irk,rv,findsun);
*/
				...
				else if (Geostationary(indx)==1)
				{
					printf("Predict - Geostationary Satellite.");
					printf("\n");
					fprintf(fd,"Geostationary");
				}
				break;
			}
		}
